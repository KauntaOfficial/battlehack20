...
def turn():
    ...
    if robottype == RobotType.PAWN:
        ...
        if check_space_wrapper(row + (forward * -1), col + 1, board_size) != team and check_space_wrapper(
                row + (forward * -1), col - 1, board_size) != team:
            push = False
        ...
        if push and check_space_wrapper(row + (forward * -1), col, board_size) == team:
            reinforce = False
            ...
            if not check_space(vert, col):
                if col_to_place == -1:
                    ...
                    max_col_threat = threat_level[col]
                        ...
                        threat_level[col] == max_col_threat and (
                        ally_pawns[col] < ally_pawns[col_to_place] or pressures[col] > pressures[col_to_place])):
                    col_to_place = col
                    max_col_threat = threat_level[col]
