...
def turn():
    ...
    if robottype == RobotType.PAWN:
        ...
        diffs = [0 for i in range(0, board_size)]
        diffs_summed = [0 for i in range(0, board_size)]
        ...
        for i in range(0, board_size):
            if diffs[i] == 100:
                diffs_summed[i] = 100
                continue
            diffs_summed[i] = diffs_summed[i] + diffs[i]
            if i > 0:
                diffs_summed[i] = diffs_summed[i] + diffs[i - 1]
            if i < board_size - 1:
                diffs_summed[i] = diffs_summed[i] + diffs[i + 1]

        for i, val in enumerate(diffs_summed):
            dlog(str(val))
            if val == 100:
                continue
            if val < diff and not check_space(vert, i):
                col_to_place = i
                diff = val
