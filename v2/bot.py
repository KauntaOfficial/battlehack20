...
def turn():
    ...
    if robottype == RobotType.PAWN:
        ...
        # - V2: DO place if row is won & adjacent rows aren't won to conquer more rows
        if team == Team.WHITE:
            forward = 1
        else:
            forward = -1
        ...
        # If you aren't losing too badly
        # Also for future v3, try to setup good trades
        in_row = -1
        push_option = False
        aggressive_option = -1
        aggressive_fill = 17
        ...
        if diff > -1:
            for col in range(0, 15):
                if check_space(opp, col) == team:
                    if (col > 0 and check_space(opp, col - 1) != team) or (
                            col < 15 and check_space(opp, col + 1) != team) and not check_space(vert, col):
                        opponent_count = 0
                        for row in range(0, 15):
                            if check_space(row, col) == opp_team:
                                opponent_count += 1
                        if opponent_count < in_row or in_row == -1:
                            push_option = True
                            col_to_place = col
                            in_row = opponent_count
                else:
                    col_empty = True
                    for row in range(0, 15):
                        if check_space(row, col):
                            col_empty = False;
                    if col_empty and not check_space(vert, col):
                        aggressive_option = col

        if not push_option and aggressive_option != -1:
            col_to_place = aggressive_option
